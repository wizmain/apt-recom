"""상가 시설(facilities STORE_*) + summary 4 subtype 을 로컬 → Railway 벌크 동기화.
...
대표 점포 선정이 로컬/prod 간 어긋나는 충돌(리뷰 지적)도 원천 회피된다.
...
사용 (기본 dry-run):
  .venv/bin/python scripts/push_store_facilities_to_railway.py
  .venv/bin/python scripts/push_store_facilities_to_railway.py --apply
"""
...
STORE_SUBTYPES = ["cafe", "kids_cafe", "pet_shop", "fitness"]
...
def main() -> None:
    ...
    lcur.execute(
        ...
        "WHERE facility_subtype = ANY(%s) AND facility_id LIKE 'STORE\\_%%'",
        [STORE_SUBTYPES],
    )
    ...
    try:
        rcur = railway.cursor()
        rcur.execute(
            "DELETE FROM facilities WHERE facility_subtype = ANY(%s)", [STORE_SUBTYPES]
        )
