...
### 거래 수집의 시간 예산 (2026-08-17)

거래 수집은 **35분 예산**(`COLLECT_BUDGET_MINUTES`) 안에서만 돌고, 초과하면 남은 시군구를
남긴 채 **정상 반환**한다. 예외를 올리거나 프로세스가 죽으면 메모리에 모아둔 수집분이 전량
날아가기 때문이다 — 적재는 수집이 반환한 **뒤에** 일어난다. 월 단위 재수집은 적재 시점의
dedupe 로 멱등이라, 남긴 몫은 다음 런이 그대로 이어받는다.

**왜 이 장치가 생겼나.** 같은 작업량(254 시군구 × 1개월)이 8/16 에는 수집 9분 12초로 끝났는데
8/17 에는 59분을 넘겨 60분 벽에서 강제 종료됐다. 작업량 증가가 아니라 **실패 경로가 비싸서**
생기는 폭발이었다:

| | 값 |
|---|---|
| 정상 콜 | 1.09초 (508콜 / 552초) |
| 실패 콜 (이전: timeout 30 × retries 3 + 백오프) | **92초** |
| 60분 초과 조건 | 508콜 중 **33콜(6%)** 만 재시도 소진 |

세 가지를 함께 바꿨다.

- **실패를 보이게** — `_call_api` 가 실패 사유를 로그하고 `CollectStats` 가 실패 콜을 집계한다.
  이전에는 예외를 삼키고 `None` → `[]` 로 흡수해 **로그가 한 줄도 남지 않았다.** 성공으로 끝난
  런조차 일부 시군구를 통째로 빠뜨렸는지 사후 확인이 불가능했다.
- **실패 비용 축소** — `timeout 15` × `retries 2` + 백오프 → 최악 콜 **31초**(이전 92초).
- **시작 오프셋 회전** — 시군구 코드가 오름차순이라 예산에 걸려 잘리는 쪽은 **항상 비수도권**이다.
  고정되면 그 지역만 결손이 누적되므로 런마다 시작 지점을 옮긴다(날짜+오전/오후 기반, 상태 저장 없음).

**`timeout-minutes` 는 60을 유지한다.** 올리는 건 증상 처리다 — 루프에 상한이 없으면 120분도
넘길 수 있고, 넘기면 여전히 전량 손실이다. 예산 35분 + 체크 간격 초과분 최대 1분(시군구당 2콜 ×
31초) + 후속 단계 약 7분 = **약 43분**이라 60분 안에 여유가 있다.

실패율이 `COLLECT_FAILURE_WARN_RATIO`(5%)를 넘으면 수집 요약에 경고가 남는다 — 예산 초과의
선행 지표다. 배치 요약의 `거래 데이터 수집` 행에도 `예산 초과 N/M 지점 중단 · 실패 N콜(N%)`
형태로 실린다(설계된 동작이라 실패로 올리지 않는다).
