...
## 게시 실패처럼 보이는 성공 (2026-08-05 수정)
`media_publish` 직후 permalink 조회가 **rate limit 403** 으로 막히는 일이 3일 연속(8/3·8/4·8/5)
있었다. 게시는 성공했는데 로그가 `published_pending` 에 멈춰, 매번 원격을 확인해 수기 보정했다.

→ **permalink 조회 실패는 더 이상 발행을 실패로 만들지 않는다.** `published` 를 기록하고 사유를
`permalink_error` 로 남긴다(`api._try_fetch_permalink`). 출력에 링크 대신 media_id 가 찍히면 정상이며,
링크는 나중에 `--check {slug}` 로 확인한다.

**원칙**: 게시 성공 사실의 기록이 부가 정보 조회 실패로 유실되면 안 된다. 게시 이후 단계에서
실패했는데 로그가 pending 이면, 재실행 전에 반드시 `--check` 로 실게시 여부를 먼저 본다
(`--force` 재실행은 자식 컨테이너부터 다시 만들어 중복 게시가 된다).
