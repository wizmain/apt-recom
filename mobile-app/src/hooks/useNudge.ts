...
import type { ScoredApartment, MapBounds, ApartmentFilters } from '../types/apartment';
...
export function useNudge() {
  ...
  const fetchWeights = useCallback(async () => {
    try {
      const res = await api.get<Record<string, Record<string, number>>>('/api/nudge/weights');
  ...
  const scoreApartments = useCallback(
    async (
      ...
      try {
        setLoading(true);
        const body: Record<string, unknown> = { nudges, weights, top_n: topN };
        if (bounds) {
