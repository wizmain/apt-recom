...
export default function NudgeBar({ onOpenSettings, onOpenFilter, filterCount }: NudgeBarProps) {
  const selectedNudges = useAppStore((s) => s.selectedNudges);
  const toggleNudge = useAppStore((s) => s.toggleNudge);
  const clearSelectedNudges = useAppStore((s) => s.clearSelectedNudges);
  const searchKeywords = useAppStore((s) => s.searchKeywords);
  const keywordLabels = useAppStore((s) => s.keywordLabels);
  ...
  // 기존 App.tsx onClearRegion 동치 (region 클리어 후 키워드가 비어있으면 nudge 도 리셋)
  const handleClearRegion = () => {
    clearRegion();
    if (searchKeywords.length === 0) clearSelectedNudges();
  };
  // 기존 onSelectApartment 동치: focus + select (+ keyword 추가는 MapControls 내부 SearchBox 에서 이미 처리)
  const handleSelectApartment = (pnu: string, lat: number, lng: number, name: string) => {
    focusApartment({ pnu, lat, lng, name });
    selectApartment(pnu);
  };
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-10 bg-white/95 backdrop-blur-sm shadow-sm">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 h-12 sm:h-14">
        ...
        <ViewTabs viewMode={viewMode} onViewChange={switchView} />
