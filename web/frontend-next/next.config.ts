...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [
      ...
      // 참고: 정적 파일(public/)의 Content-Type charset·CORS 는 headers() 로
      // 제어되지 않는다(headers() 는 Next.js 라우트 전용). llms.txt / skill.md /
      // mcp server-card 등 정적 자산 헤더는 `public/_headers` 에서 지정한다.
    ];
  },
};
