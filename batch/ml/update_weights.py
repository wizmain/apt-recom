...
정책: learned_weights.json 에 없는 subtype(예: score_* pseudo-subtype, 신규
파생 지표)은 ML 신호가 없으므로 블렌딩하지 않고 현재 가중치를 그대로 유지한다
(임의 기본값으로 블렌딩하면 실제 학습 신호가 희석되는 것을 방지). 정규화는
유지된 subtype 을 포함해 전체 넛지에 동일하게 적용한다.
...
def main():
    ...
    parser.add_argument(
        "--ml-ratio", type=float, default=0.4, help="ML 가중치 반영 비율 (기본 0.4)"
    )
    ...
    for nudge_id, subtypes in current_weights.items():
        ...
        new_weights = {}
        # ML 미학습 subtype(score_* pseudo-subtype, 신규 파생 지표 등) — 블렌딩 없이 유지
        kept_subtypes: set[str] = set()
        for subtype, cur_w in subtypes.items():
            ml_w = ml_weights.get(subtype)
            if ml_w is None:
                new_weights[subtype] = cur_w
                kept_subtypes.add(subtype)
            else:
