...
def collect_from_api(conn=None, logger=None, search_date=None, dry_run=False, limit=0):
    ...
    logger.info(
        f"관리비 API 수집 시작: searchDate={ym}, limit={limit}, dry_run={dry_run}"
    )
    ...
    for i, (kapt_code, pnu, hhld) in enumerate(targets):
        ...
        if loaded % 5000 == 0:
            if not dry_run:
                conn.commit()
            logger.info(
                f"  진행: {loaded:,}건 적재 (스킵 {skipped:,}, 실패 {failed:,})"
            )
    ...
    logger.info(
        f"관리비 API 수집 완료: {loaded:,}건 적재 (스킵 {skipped:,}, 실패 {failed:,})"
    )
...
def collect_from_xlsx(
    ...
    for r in query_all(
        ...
        kapt_pnu[str(r["kapt_code"]).strip()] = (
            r["pnu"],
            r["kapt_hhld"],
            r["apts_hhld"],
        )
    ...
    for source_row, (_, row) in enumerate(df_cost.iterrows(), start=3):
        raw_kapt_code = row.get("단지코드")
        kapt_code = "" if raw_kapt_code is None else str(raw_kapt_code).strip()
        apt_name = "" if row.get("단지명") is None else str(row.get("단지명")).strip()
        pnu_info = kapt_pnu.get(kapt_code)
        if not pnu_info:
            skipped += 1
            if not kapt_code:
                reason = "missing_kapt_code"
            elif kapt_code in mapped_kapt_codes:
                reason = "no_apartments_join"
            else:
                reason = "no_apt_kapt_info_mapping"
            skipped_rows.append(
                {
                    ...
                    "apt_name": apt_name,
                    "year_month": ""
                    if row.get("발생년월(YYYYMM)") is None
                    else str(row.get("발생년월(YYYYMM)")),
                    "reason": reason,
                }
            )
            continue
        ...
        raw_ym = row.get("발생년월(YYYYMM)", 0)
        ym = str(int(raw_ym or 0))
        if len(ym) != 6:
            skipped += 1
            skipped_rows.append(
                {
                    ...
                    "kapt_code": kapt_code,
                    "apt_name": apt_name,
                    "year_month": "" if raw_ym is None else str(raw_ym),
    ...
    if skipped_rows:
        ...
        date_hint = (
            cost_paths[0].name.split("_")[0]
            if cost_paths
            else datetime.now().strftime("%Y%m%d")
        )
        report_path = report_dir / f"kapt_mgmt_cost_skipped_{target}_{date_hint}.csv"
        pd.DataFrame(skipped_rows).to_csv(
            report_path, index=False, encoding="utf-8-sig"
        )
        logger.info(f"  스킵 리포트: {report_path}")
    ...
    return {
        "loaded": loaded,
        "skipped": skipped,
        "skip_report": str(report_path) if report_path else None,
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=0, help="테스트용 최대 수집 건수 (0=전체)"
    )
    parser.add_argument(
        "--target",
    ...
    if args.source == "xlsx":
        ...
        for t in targets:
            collect_from_xlsx(
                target=t,
                cost_files=args.cost_file,
                area_xlsx=args.area_file,
