...
from batch.trade.collect_trades import (
    _call_api,
    _parse_xml,
    TRADE_COL_MAP,
    RENT_COL_MAP,
...
def main():
    parser = argparse.ArgumentParser(
        description="비수도권 과거 3년 거래 데이터 초기 수집"
    )
    parser.add_argument(
        "--max-calls", type=int, default=900, help="이번 실행 최대 API 호출 수"
    )
    parser.add_argument(
        "--skip-enrich",
        action="store_true",
        help="신규 아파트 보충 생략 (별도 워크플로에서 실행)",
    )
    ...
    logger.info(
        f"전체: {total_pairs}쌍, 완료: {len(completed)}쌍, 남은: {len(remaining)}쌍"
    )
    ...
    for code, month in remaining:
        ...
        if pair_count % 50 == 0:
            ...
            logger.info(
                f"  진행: {pair_count}쌍 완료 ({call_count}콜), 매매 {len(trade_rows)}건, 전월세 {len(rent_rows)}건"
            )
    ...
    logger.info(
        f"전체 진행: {len(completed)}/{total_pairs}쌍 ({len(completed) * 100 // total_pairs}%)"
    )
